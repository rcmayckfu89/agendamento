...
export const Agenda: React.FC = () => {
    ...
    const handleSaveAppointment = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setErrorMessage("Erro ao salvar agendamento. Verifique a conexão e tente novamente.");
        }
    };
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between p-3 md:p-4 border-b border-border bg-muted/20 gap-3">
                    <div className="flex items-center gap-3 md:gap-4 w-full sm:w-auto justify-between sm:justify-start">
                        ...
                        <div className="flex items-center gap-1 bg-background rounded-lg border border-border p-0.5 shadow-sm">
                            <button onClick={() => { const d = new Date(currentDate); d.setDate(d.getDate() - 7); setCurrentDate(d); }} className="p-1 px-2 rounded-lg hover:bg-accent hover:text-white transition-colors"><span className="material-symbols-outlined text-base md:text-lg">chevron_left</span></button>
                            ...
                            <button onClick={() => { const d = new Date(currentDate); d.setDate(d.getDate() + 7); setCurrentDate(d); }} className="p-1 px-2 rounded-lg hover:bg-accent hover:text-white transition-colors"><span className="material-symbols-outlined text-base md:text-lg">chevron_right</span></button>
                        </div>
                    </div>
                                    ...
                                    <div className="relative">
                                        ...
                                        {showPatientList && (
                                            <div className="absolute z-10 w-full mt-1 bg-card border border-border rounded-lg shadow-2xl max-h-52 overflow-auto custom-scrollbar">
                                                {(() => {
                                                    const filtered = patients.filter(p => p.name.toLowerCase().includes(patientSearchTerm.toLowerCase()));
